<script setup lang="ts">

</script>

<template>
 Hallo Dashboard

  <div class="q-pa-md q-gutter-sm">
    <q-btn outline rounded color="primary" label="Taschenrechner" />
    <q-btn unelevated round color="primary" disable icon="card_giftcard" />
    <q-btn outline rounded color="primary" label="Store" />
    <q-btn unelevated round color="primary" disable icon="card_giftcard" />
    <q-btn outline rounded color="primary" label="wikipedia" />
    <q-btn unelevated round color="primary" disable icon="card_giftcard" />
    <q-btn outline rounded color="primary" label="google übersetzer" />
    <q-btn unelevated round color="primary" disable icon="card_giftcard" />
    <q-btn outline rounded color="primary" label="google" />

  </div>
</template>

<style scoped>

</style>